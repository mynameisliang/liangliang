...
<script type="text/javascript">
	function _search() {
		 ...
		 form.action="${rootPath}user/find?search="+a;
		 form.submit(); 
	}  
</script>
</head>
	                      ...
	                      <ul class="nav pull-left">
	                      	<ul class="nav nvabar-nav">
                                	...
                                	<c:if test="${user.type=='管理员'}">
                                		<li class="active"><a href="${rootPath}user/find">用户管理</a></li>
                                	</c:if>
	...
	<div class="wrapper"> 
		<div class="container" style="padding-top:30px">
			...
			<div class="input-group" style="float:right">
	         	...
	         	<table class="table table-striped table-hover ">	  
				  ...
				  <tbody>
				  	<c:forEach var="u"   items="${pc.data}"   varStatus="i">
					      ...
					      <td>${u.username}</td>
					      <td>${u.type}</td>
					      <td><a href="${rootPath}user/toUpdate?id=${u.id}">修改</a></td>
					      <td><a href="${rootPath}user/delete?id=${u.id}">删除</a></td>		      
					    </tr>
				  	</c:forEach>
				...
				<div class="list-page">
					<ul class="pagination">
						<li><a href="${rootPath}user/find?curr=${pc.prePage}&search=${search}">上一页</a></li>
						<c:if test="${1 < pc.currentPage -3}">
							<li><a href="${rootPath}user/find?curr=1&search=${search}">1</a></li>
						</c:if>
						...
						<c:forEach var="i"
							...
							<c:choose>
								<c:when test="${i>0 && i == pc.currentPage }">
									<li class="active"><a
										href="${rootPath}user/find?curr=${i}&search=${search}">${i}</a></li>
								</c:when>
			
								<c:when test="${i>0 && i != postPS.currentPage }">
									<li><a href="${rootPath}user/find?curr=${i}&search=${search}">${i}</a></li>
								</c:when>
							</c:choose>
						</c:forEach>
						<li><a
							href="${rootPath}user/find?curr=${pc.nextPage}&search=${search}">下一页</a></li>
					</ul>		
				</div>
