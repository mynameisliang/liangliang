...
<script type="text/javascript">
	function _search() {
		 ...
		 form.action="${rootPath}node/find?search="+a;
		 form.submit(); 
	}  
</script>
</head>
	                      ...
	                      <ul class="nav pull-left">
	                      	<ul class="nav nvabar-nav">
	                                  <li><a href="${rootPath}weight/toIndex">首页</a></li>
	                                    <li class="active"><a href="${rootPath}node/find">节点管理</a></li>
	...
	<div class="wrapper"> 
		<div class="container" style="padding-top:30px">
			...
			<div class="input-group" style="float:right">
				<form id="_searchForm" action="${rootPath}node/find" method="get" onsubmit="return _search();">	
	         	...
	         	<table class="table table-striped table-hover ">	  
				  ...
				  <tbody>
				  	<c:forEach var="n"   items="${pc.data}"   varStatus="i">
					      ...
					      <td>${n.x}</td>
					      <td>${n.y}</td>
					      <td><a href="${rootPath}node/delete?id=${n.id}">删除</a></td>		      
					    </tr>
				  	</c:forEach>
				...
				<div class="list-page">
					<ul class="pagination">
						<li><a href="${rootPath}node/find?curr=${pc.prePage}&search=${search}">上一页</a></li>
						<c:if test="${1 < pc.currentPage -3}">
							<li><a href="${rootPath}node/find?curr=1&search=${search}">1</a></li>
						</c:if>
						...
						<c:forEach var="i"
							...
							<c:choose>
								<c:when test="${i>0 && i == pc.currentPage }">
									<li class="active"><a
										href="${rootPath}node/find?curr=${i}&search=${search}">${i}</a></li>
								</c:when>
			
								<c:when test="${i>0 && i != postPS.currentPage }">
									<li><a href="${rootPath}node/find?curr=${i}&search=${search}">${i}</a></li>
								</c:when>
							</c:choose>
						</c:forEach>
						<li><a
							href="${rootPath}node/find?curr=${pc.nextPage}&search=${search}">下一页</a></li>
					</ul>		
				</div>
